...
iterLimit = 5000; % in each episode, the limit of number of actions a robot can take. i.e. one episode lasts at most 200 seconds.
...
w = [1000, -5, -5, -8, -2, -1, 0, -2, -4, -1, -200];
reward = @(s, a, w) dot(w(1:4), [s(1), s(5)^2 *s(2)^3, s(5)^2 *s(3)*s(2)^3, s(4)^2]) + w(4+a) + w(10) + s(5)^2 * w(11) * (s(2)==3); % a=3-> w(7)=0
% r = w1 * state1 + w2 * (state5 * state2 ^3) + w3 * (state5 * state3 * state2 ^3) + w4 * state4 ^2 + [w5, w6, w7, w8, w9](a) + w10 
...
% w4: negative reward for velocity being deviated from a nominal velocity
% w5, w6, w7, w8, w9: negative reward for driving the robot. Acceleration cost always higher than deceleration at the same scale.
% w10: negative reward for time cost
...
% state5 - 1: < nHumansAround(1) humans inside rRobot(3)
...
% r = w1 * state1 + w2 * (state2 ^3) + w3 * (state3 * state2 ^3) + w4 * state4 ^2 + [w5, w6, w7, w8, w9](a) + w10 
...
while totalIter <= totalEpisodes
    ...
    while iter < iterLimit
        ...
        if rand() < epsilon % random choice
            ...
            [tmp, atPlus] = max(Q(s(1)+1, s(2)+1, s(3)+1, s(4)+1, s(5), :));% we need index
        end
        
        tPlusIdx = sub2ind(size(Q), s(1)+1, s(2)+1, s(3)+1, s(4)+1, s(5), atPlus);
        QtPlus = Q(tPlusIdx);
        
        tIdx = sub2ind(size(Q), st(1)+1, st(2)+1, st(3)+1, st(4)+1, s(5), at);
        Qt = Q(tIdx);
            ...
            for s2 = 1:4
                for s3=1:2
                    for s4=1:3
                        for s5=1:3
                            for aa=1:5
                                tmpIdx = sub2ind(size(Q), s1,s2,s3,s4,s5,aa);
