...
% load('test_1.mat') % good
% load('test_2.mat') % bad
% load('test_3.mat') % bad
load('test_4.mat') % good
% load('test_7_edecay_onePolicy_10000.mat') % bad
...
while totalIter <= totalTests
    ...
    if mod(totalIter, 100) == 0
        fprintf('%.0f tests have beed done.', totalIter)
    end
    disp(totalIter);
...
humanPosHValid = humanPosHValid(:,1:50,:);
robotPos = robotPos(1:50, :);
...
for i = 1:nHumans
    ...
    plot([humanStart(i,1) humanEnd(i,1)], [humanStart(i,2) humanEnd(i,2)], ':', 'Color', 'b','LineWidth',2)
    
    
    %// radius
    rTmp = 15;
    posStart = [humanStart(i,:)-rTmp 2*rTmp 2*rTmp];
    startPoint = rectangle('Position',posStart,'Curvature',[1 1], 'FaceColor', 'green', 'Edgecolor','none');
    posEnd = [humanEnd(i,:)-rTmp 2*rTmp 2*rTmp];
    endPoint = rectangle('Position',posEnd,'Curvature',[1 1], 'FaceColor', 'black', 'Edgecolor','none');
...
% patch('XData',x*30,'YData',y*30,'FaceColor','red','Parent',g)
...
% x = [-1, 1/3, 1/3, 1, 1/3, 1/3, -1];
% y = [-1/3,-1/3,-1/2, 0, 1/2, 1/3, 1/3];
% robotFilledCircle = rectangle('Position',posEnd,'Curvature',[1 1], 'FaceColor', 'black', 'Edgecolor','none');
% g = hgtransform;
% patch('XData',x*30,'YData',y*30,'FaceColor','red','Parent',g)
% pt1 = [0 0 0];
% pt2 = [endpoint 0 0];
