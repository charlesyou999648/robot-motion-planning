...
totalEpisodes = 10000; % number of episodes in training process
iterLimit = 2000; % in each episode, the limit of number of actions a robot can take. i.e. one episode lasts at most 200 seconds.
...
nHumans = 5; % number of humans walking in the hall way
earliestStartTime = 0;
...
w = [1000, -2, -2, -8, -2, -1, 0, -2, -4, -1, -200];
reward = @(s, a, w) dot(w(1:4), [s(1), s(2)^3, s(3)*s(2)^3, s(4)^2]) + w(4+a) + w(10) + w(11) * (s(2)==3); % a=3-> w(7)=0
...
while totalIter <= totalEpisodes
    ...
    while iter < iterLimit
        ...
        tPlusIdx = sub2ind(size(Q), s(1)+1, s(2)+1, s(3)+1, s(4)+1, atPlus);
        QtPlus = Q(tPlusIdx);
        
        tIdx = sub2ind(size(Q), st(1)+1, st(2)+1, st(3)+1, st(4)+1, at);
        Qt = Q(tIdx);
